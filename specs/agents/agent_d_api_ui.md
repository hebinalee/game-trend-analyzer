...
**이슈 목록 페이지**
- 플랫폼 필터 (상위): 전체 / PC(Steam) / 모바일 — 레이블+좌측 세로선으로 계층 표현
- 세부 필터 (하위, 플랫폼 필터 아래 들여쓰기): 심각도 탭(전체/CRITICAL/WARNING/미확인) + 게임 드롭다운
- 플랫폼 변경 시 게임 필터 초기화
...
- 플랫폼 필터: 전체 / PC(Steam) / 모바일 — 플랫폼 변경 시 선택 게임 및 결과 초기화
- 게임 선택: `GameSelector` 컴포넌트 (체크박스 그리드, 최대 4개)
- 날짜 선택 + 비교하기 버튼
...
- 플랫폼 필터: 전체 / PC(Steam) / 모바일 — 각 버튼에 해당 게임 수 표시

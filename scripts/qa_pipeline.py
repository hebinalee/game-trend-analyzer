...
async def run_demo(game_data: dict, client: anthropic.AsyncAnthropic) -> None:
    print(f"\n{'='*60}")
...
async def run_interactive(game_data: dict, client: anthropic.AsyncAnthropic) -> None:
    print(f"\n{'='*60}")
...
async def main() -> None:
    ...
    if args.interactive:
        await run_interactive(game_data, ai_client)
    else:
        await run_demo(game_data, ai_client)
